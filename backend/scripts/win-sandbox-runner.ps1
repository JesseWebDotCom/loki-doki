...
# as the restricted `maipai-coding` user via .NET Process.Start with credentials —
...
#   MAIPAI_SB_MODE        probe (wait, exit with child's code) | spawn (detach, print PID)
#   MAIPAI_SB_BLOB        path to the DPAPI credential blob
#   MAIPAI_SB_USER        sandbox account name
#   MAIPAI_SB_EXE         executable to run
#   MAIPAI_SB_ARGS        JSON array of arguments
#   MAIPAI_SB_CWD         working directory (must be accessible to the SANDBOX user)
#   MAIPAI_SB_ENV         JSON object of environment overrides for the child
#   MAIPAI_SB_CLEAN       '1' = start from a minimal system env instead of inheriting
#                           this process's (the backend's env carries real secrets that
#                           must never reach a process the sandbox user can inspect)
#   MAIPAI_SB_TIMEOUT_MS  probe mode: kill + exit 124 after this long

$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Security

$blob = [System.IO.File]::ReadAllBytes($env:MAIPAI_SB_BLOB)
...
$argList = @()
if ($env:MAIPAI_SB_ARGS) { $argList = @(ConvertFrom-Json $env:MAIPAI_SB_ARGS) }

$psi = New-Object System.Diagnostics.ProcessStartInfo
$psi.FileName = $env:MAIPAI_SB_EXE
$psi.Arguments = (($argList | ForEach-Object { Quote-Arg $_ }) -join ' ')
$psi.UserName = $env:MAIPAI_SB_USER
$psi.Password = $secure
$psi.Domain = '.'
...
if ($env:MAIPAI_SB_CWD) { $psi.WorkingDirectory = $env:MAIPAI_SB_CWD }
if ($env:MAIPAI_SB_CLEAN -eq '1') {
...
if ($env:MAIPAI_SB_ENV) {
  $overrides = ConvertFrom-Json $env:MAIPAI_SB_ENV
...
foreach ($k in @('MAIPAI_SB_MODE','MAIPAI_SB_BLOB','MAIPAI_SB_USER','MAIPAI_SB_EXE','MAIPAI_SB_ARGS','MAIPAI_SB_CWD','MAIPAI_SB_ENV','MAIPAI_SB_TIMEOUT_MS')) {
  if ($psi.EnvironmentVariables.ContainsKey($k)) { $psi.EnvironmentVariables.Remove($k) }
}

$p = [System.Diagnostics.Process]::Start($psi)

if ($env:MAIPAI_SB_MODE -eq 'spawn') {
...
$timeoutMs = 30000
if ($env:MAIPAI_SB_TIMEOUT_MS) { $timeoutMs = [int]$env:MAIPAI_SB_TIMEOUT_MS }
