...
export const bookmarksLibraryTool: Tool = {
  ...
  toolDefinition: {
    ...
    function: {
      ...
      description: "Search the user's saved Bookmarks library (bookmarks + offline articles) by keyword",
      parameters: {
        type: 'object',
        required: ['query'],
  ...
  async execute(args: unknown, config?: Record<string, unknown>): Promise<ToolResult> {
    ...
    const rows = await db.select().from(bookmarks).where(and(...conds)).orderBy(desc(bookmarks.createdAt)).limit(8)
    return {
      success: true,
      data: {
        count: rows.length,
