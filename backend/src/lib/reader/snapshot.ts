...
//
// Stored under data/reader-archive/<itemId>/:
...
//
// Served (auth + ownership checked) from GET /api/reader/:id/archive/* so the snapshot
...
export const ARCHIVE_ROOT = 'reader-archive' // relative to dataDir
...
export interface SnapshotResult {
  ...
  snapshotRelDir: string // e.g. 'reader-archive/<id>' (relative to dataDir)
...
export async function capturePage(
  ...
  const snapshotRelDir = `${ARCHIVE_ROOT}/${itemId}`
  const absDir = join(dataDir, ARCHIVE_ROOT, itemId)
  ...
  const reader = extractFromHtml(html, url, {
    localizeImage: (absUrl) => {
      ...
      return local ? `/api/reader/${itemId}/archive/${local}` : null
    },
  })
