...
async function tick(): Promise<void> {
  ...
  try {
    const now = Date.now()
    const items = await db.select().from(bookmarks).where(eq(bookmarks.autoUpdate, true))
    ...
    for (const item of due) {
      await db.update(bookmarks).set({ lastCheckedAt: new Date() }).where(eq(bookmarks.id, item.id))
...
export function startBookmarkAutoUpdatePoller(): void {
  if (_timer) return
