...
adminBookmarksRouter.get('/', requireAdmin, async (c) => {
  const rows = await db.select().from(bookmarks)
    .where(and(isNull(bookmarks.ownerId), eq(bookmarks.type, 'live')))
...
adminBookmarksRouter.post('/', requireAdmin, async (c) => {
  ...
  await db.insert(bookmarks).values(item)
  return c.json({ item })
})
...
adminBookmarksRouter.patch('/:id', requireAdmin, async (c) => {
  ...
  const existing = await db.select().from(bookmarks).where(and(eq(bookmarks.id, id), isNull(bookmarks.ownerId))).then(r => r[0])
