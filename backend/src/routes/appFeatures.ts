...
const APP_FEATURE_IDS = ['bookmarks', 'home-inventory', 'notes'] as const
...
async function getFeatureMap(): Promise<Record<AppFeatureId, boolean>> {
  ...
  const map: Record<AppFeatureId, boolean> = { bookmarks: true, 'home-inventory': true, notes: true }
...
appFeaturesRouter.get('/', requireAuth, async (c) => {
  ...
  const map: Record<string, boolean> = { bookmarks: true, 'home-inventory': true, notes: true }
  for (const row of rows) {
