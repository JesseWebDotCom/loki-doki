...
const APP_FEATURE_IDS = ['links', 'home-inventory'] as const
...
async function getFeatureMap(): Promise<Record<AppFeatureId, boolean>> {
  ...
  const map: Record<AppFeatureId, boolean> = { links: true, 'home-inventory': true }
...
appFeaturesRouter.get('/', requireAuth, async (c) => {
  ...
  const map: Record<string, boolean> = { links: true, 'home-inventory': true }
  for (const row of rows) {
