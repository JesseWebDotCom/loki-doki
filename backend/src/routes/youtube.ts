...
youtubeRoute.get('/video/:videoId', async (c) => {
  ...
  if (v?.description) {
    ...
    return c.json({ videoId, title: v.title, author: v.author, channelId: v.channelId, channelThumb: await avatarFor(sub, v.channelId), description: v.description, summary: v.summary, durationSec: v.durationSec, views: v.views, positionSec, subscribed: !!sub, subscriptionId: sub?.id ?? null, isLive: false })
  }
