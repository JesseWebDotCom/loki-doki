...
def get_account(conn: sqlite3.Connection, account_id: str) -> dict[str, Any]:
    ...
    row = conn.execute(
        ...
        SELECT a.id, a.name, a.default_character_id, a.character_feature_enabled,
               p.core_safety_prompt, p.account_policy_prompt, p.proactive_chatter_enabled
        FROM accounts a
    ...
    return {
        ...
        "account_policy_prompt": str(row["account_policy_prompt"] or ""),
...
def update_prompt_policy(conn: sqlite3.Connection, account_id: str, values: dict[str, Any]) -> dict[str, Any]:
    ...
    conn.execute(
        """
        INSERT INTO account_prompt_policy (account_id, core_safety_prompt, account_policy_prompt, proactive_chatter_enabled, updated_at)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(account_id) DO UPDATE SET
            core_safety_prompt = excluded.core_safety_prompt,
            account_policy_prompt = excluded.account_policy_prompt,
        ...
        (
            ...
            str(values.get("account_policy_prompt", current["account_policy_prompt"])).strip(),
