...
async def run_pipeline_async(
    ...
    """Run the pipeline pipeline end-to-end."""
    trace = start_trace()
    ...
    # PipelineRunRequest fields) take precedence over derivation.
    ...
    # Bridge session state into context["recent_entities"] so the
...
def _run_memory_write_path(
    ...
    Memory writes are **opt-in**: the dev-tools pipeline runner enables
    ...
    existing regression suite isn't affected by storage side-effects.
...
# Tone signal → numeric sentiment: maps interaction signals to the
