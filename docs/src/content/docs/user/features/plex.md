...
If your household runs a [Plex](https://www.plex.tv) media server, MaiPai Home connects to it so the **Shows** and **Movies** apps know what's already in your library. No more paying to stream something you already own.

## What it adds

- **Plex badge**: when you look up a show or movie, MaiPai Home checks your Plex library and marks the titles you already have, so you can tell at a glance what's at home versus what needs streaming.
- **On Deck and Recently Added**: rails on the Shows home page pulled straight from your Plex server.
- **Two-way watchlist**: add a title in MaiPai Home and it lands on your Plex watchlist too, and vice versa.
- **Watched status sync**: marking something watched in MaiPai Home scrobbles it in Plex, so your progress stays the same everywhere.
- **Play here**: for movies, a "Play here" button streams straight from your Plex server inside MaiPai Home, no separate app needed.
...
MaiPai Home can give each person their own **private Plex libraries of their saved videos — one per source**: YouTube, TikTok, Vimeo, Reddit, and their own creations from the [Videos](../videos/) app. It takes a few one-time steps in **Admin → Plex**:
...
4. **Map it to Plex's path** — enter how *Plex itself* sees that same folder (its path may differ from how MaiPai Home sees it, e.g. mounted under a different drive letter or share name).
...
Streaming and library lookups happen between your MaiPai Home instance and your Plex server directly, over your own network if the server is local. Your Plex access token is never exposed to the browser. If Plex isn't configured, or you haven't linked your account, the badges and features simply don't appear.
