...
def _pinned_fetch_specs(profiles: Iterable[str], cache: Path) -> list[FetchSpec]:
    ...
    if any(p in _MAPS_PROFILES for p in profiles):
        ...
        # OSM water polygons (~880 MiB) + Monaco extract (~800 KiB).
        # Pre-seeded so ``building_streets`` + ``build-world-overview``
        # run offline; planetiler reads the zips and the pbf directly.
        for pin, label in (
            (V.NATURAL_EARTH, "natural-earth"),
            (V.OSM_WATER_POLYGONS, "water-polygons"),
            (V.MONACO_PBF, "monaco-pbf"),
        ):
            specs.append(
                FetchSpec(
        ...
        _log.info(
            ...
            "water polygons ~880 MiB + monaco ~800 KiB)"
        )
