...
from v2.orchestrator.memory.consolidation import (
...
class TestSessionState:
    def test_create_and_get_session(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
        assert isinstance(sid, int) and sid > 0
        state = store.get_session_state(sid)
        assert state == {}

    def test_set_and_get_session_state(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
        store.set_session_state(sid, {"foo": "bar"})
        assert store.get_session_state(sid) == {"foo": "bar"}

    def test_update_last_seen(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
    ...
    def test_unknown_session_returns_empty(self, store: V2MemoryStore):
        assert store.get_session_state(9999) == {}

    def test_close_session(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
...
class TestEpisodes:
    def test_write_and_get_episodes(self, store: V2MemoryStore):
    ...
    def test_episode_fts_search(self, store: V2MemoryStore):
        store.write_episode(
            owner_user_id=OWNER,
            title="Japan trip",
    ...
    def test_topic_scope_filters_episodes(self, store: V2MemoryStore):
        store.write_episode(
            owner_user_id=OWNER,
            title="Japan trip",
    ...
    def test_count_episodes_with_claim(self, store: V2MemoryStore):
        for i in range(3):
            sid = store.create_session(OWNER)
            store.write_episode(
                owner_user_id=OWNER,
...
class TestTriggeredConsolidation:
    def test_below_threshold_not_triggered(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
    ...
    def test_reaches_threshold_triggers(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
...
class TestCrossSessionPromotion:
    def test_promotion_after_3_sessions(self, store: V2MemoryStore):
    ...
    def test_no_promotion_below_threshold(self, store: V2MemoryStore):
...
class TestSummarizationQueue:
    def test_queue_and_drain(self, store: V2MemoryStore):
        reset_queue()
        sid = store.create_session(OWNER)
...
class TestSlotAssembly:
    ...
    def test_assemble_recent_context_end_to_end(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
    ...
    def test_assemble_relevant_episodes_end_to_end(self, store: V2MemoryStore):
        store.write_episode(
            owner_user_id=OWNER,
            title="Japan trip",
...
class TestLatency:
    def test_session_state_lookup_under_50ms_p95(self, store: V2MemoryStore):
...
class TestPipelineIntegration:
    def test_session_created_when_memory_enabled(self, store: V2MemoryStore):
    ...
    def test_session_not_created_when_memory_disabled(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import run_pipeline
    ...
    def test_session_close_queues_summarization(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import run_pipeline
    ...
    def test_need_session_context_gates_slot(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import run_pipeline
    ...
    def test_need_episode_gates_slot(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import run_pipeline
    ...
    def test_no_slots_when_flags_not_set(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import run_pipeline
...
class TestStaleContextDecay:
    def test_decay_drops_old_entries(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
    ...
    def test_decay_keeps_recent_entries(self, store: V2MemoryStore):
        sid = store.create_session(OWNER)
    ...
    def test_decay_ignores_entries_without_turn(self, store: V2MemoryStore):
...
class TestSessionStateBridge:
    def test_bridge_populates_recent_entities(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import _bridge_session_state_to_recent_entities
    ...
    def test_bridge_does_not_duplicate(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import _bridge_session_state_to_recent_entities
...
class TestAutoRaiseNeedSessionContext:
    def test_auto_raise_on_unresolved_referent(self):
        from v2.orchestrator.core.pipeline import _auto_raise_need_session_context
        from v2.orchestrator.core.types import ResolutionResult
...
class TestPhaseConstants:
    ...
    def test_active_phase_is_m4(self):
        assert ACTIVE_PHASE_ID == "m4"
        assert ACTIVE_PHASE_LABEL == "M4"
