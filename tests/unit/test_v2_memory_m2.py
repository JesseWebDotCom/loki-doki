...
RECALL_CORPUS = REPO_ROOT / "tests" / "fixtures" / "v2_memory_recall_corpus.json"


@pytest.fixture()
def store(tmp_path: Path) -> V2MemoryStore:
    s = V2MemoryStore(tmp_path / "v2_memory_m2.sqlite")
    yield s
    s.close()
...
def _seed_writes(store: V2MemoryStore, owner_user_id: int, writes: list[dict]) -> None:
    for w in writes:
...
def test_m2_fts5_table_exists_after_bootstrap(store: V2MemoryStore) -> None:
...
def test_m2_fts5_triggers_keep_index_in_sync(store: V2MemoryStore) -> None:
...
def test_m2_fts5_supersession_swaps_in_new_value(store: V2MemoryStore) -> None:
...
def test_m2_pipeline_does_not_read_when_need_preference_false(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m2_no_read.sqlite")
...
def test_m2_pipeline_reads_when_need_preference_true(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m2_yes_read.sqlite")
    test_store.write_semantic_fact(
        MemoryCandidate(
            subject="self",
            predicate="favorite_color",
...
def test_m2_render_user_facts_clips_to_budget(store: V2MemoryStore) -> None:
...
def test_m2_recall_corpus_pass_rate(recall_corpus, tmp_path: Path) -> None:
    ...
    failures: list[str] = []
    test_store = V2MemoryStore(tmp_path / "m2_corpus.sqlite")
    try:
...
def test_m2_reader_latency_p95_under_100ms_warm(store: V2MemoryStore) -> None:
...
def test_m2_cross_user_isolation_in_reader(store: V2MemoryStore) -> None:
    store.write_semantic_fact(
        MemoryCandidate(
            subject="self",
            predicate="favorite_food",
...
def test_m2_dev_v2_status_active_phase_is_m2() -> None:
    from lokidoki.api.routes.dev import _v2_memory_status

    payload = _v2_memory_status()
    assert payload["active_phase"]["id"] == "m2"
    ...
    assert "fts5" in deliverables or "read" in deliverables
...
def test_m2_pipeline_end_to_end_recall_after_write(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m2_e2e.sqlite")
    try:
