...
from lokidoki.orchestrator.memory.aggregation import (
...
class TestBehaviorEvents:
    ...
    def test_read_with_since_filter(self, store: MemoryStore):
    ...
    def test_read_with_event_type_filter(self, store: MemoryStore):
...
class TestUserProfile:
    ...
    def test_set_and_get_telemetry(self, store: MemoryStore):
        store.set_user_telemetry(OWNER, {"total_turns": 42})
        profile = store.get_user_profile(OWNER)
        assert profile["telemetry"]["total_turns"] == 42

    def test_style_update_preserves_telemetry(self, store: MemoryStore):
    ...
    def test_style_overwrite(self, store: MemoryStore):
...
class TestOptOut:
    ...
    def test_opt_out_prevents_reads(self, store: MemoryStore):
...
class TestAggregation:
    def test_no_events_noop(self, store: MemoryStore):
        result = run_aggregation(store, OWNER)
        assert result["events_processed"] == 0
        assert result["style_updated"] is False

    def test_below_min_events_no_style(self, store: MemoryStore):
    ...
    def test_derives_verbosity_concise(self, store: MemoryStore):
        for _ in range(10):
            store.write_behavior_event(
                OWNER, event_type="turn",
    ...
    def test_derives_verbosity_detailed(self, store: MemoryStore):
        for _ in range(10):
            store.write_behavior_event(
                OWNER, event_type="turn",
    ...
    def test_derives_preferred_modality(self, store: MemoryStore):
        for _ in range(7):
            store.write_behavior_event(
                OWNER, event_type="turn",
    ...
    def test_updates_telemetry_counters(self, store: MemoryStore):
        for _ in range(5):
            store.write_behavior_event(
                OWNER, event_type="turn",
    ...
    def test_aggregation_latency(self, store: MemoryStore):
    ...
    def test_stable_descriptors_across_runs(self, store: MemoryStore):
...
class TestUserStyleSlot:
    ...
    def test_assemble_empty_profile(self, store: MemoryStore):
        rendered, style = assemble_user_style_slot(store=store, owner_user_id=OWNER)
        assert rendered == ""
        assert style == {}
...
class TestTier7bLeak:
    ...
    def test_telemetry_not_in_combine_prompt(self, store: MemoryStore):
        from lokidoki.orchestrator.fallbacks.prompts import COMBINE_PROMPT, DIRECT_CHAT_PROMPT
    ...
    def test_telemetry_not_in_rendered_slot(self, store: MemoryStore):
        store.set_user_style(OWNER, {"tone": "casual"})
        store.set_user_telemetry(OWNER, {
            "total_turns": 999,
            "opted_out": False,
...
class TestNeedRoutineDerivation:
    def test_direct_chat_triggers(self):
        from lokidoki.orchestrator.pipeline.derivations import derive_need_flags
        from lokidoki.orchestrator.core.types import ParsedInput, RequestChunk, ChunkExtraction, RouteMatch
