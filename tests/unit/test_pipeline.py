...
from lokidoki.orchestrator.core.pipeline import run_pipeline, run_pipeline_async
...
def test_normalizer_cleans_quotes_and_spaces():
...
def test_splitter_splits_obvious_compound_request():
...
def test_pipeline_handles_fast_lane_spelling_request():
...
def test_pipeline_handles_obvious_compound_request_end_to_end():
...
def test_pipeline_extracts_and_resolves_chunk_context():
...
def test_pipeline_builds_request_spec_and_trace_summary():
    result = run_pipeline("hello and how do you spell necessary")
...
def test_pipeline_resolves_recent_movie_from_context():
    result = run_pipeline(
        "what was that movie",
        context={
            "recent_entities": [
...
def test_pipeline_marks_recent_movie_request_unresolved_without_context():
...
def test_pipeline_marks_recent_movie_request_ambiguous_with_multiple_candidates():
    result = run_pipeline(
        "what was that movie",
        context={
            "recent_entities": [
...
def test_pipeline_trace_contains_per_chunk_stage_details():
...
@pytest.mark.anyio
async def test_async_pipeline_matches_sync_shape():
...
@pytest.mark.anyio
async def test_async_pipeline_resolves_recent_movie_from_context():
