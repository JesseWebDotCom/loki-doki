...
from v2.orchestrator.memory.classifier import classify_candidate
from v2.orchestrator.memory.extractor import ExtractionContext, extract_candidates
from v2.orchestrator.memory.gates import GateName, run_gate_chain
from v2.orchestrator.memory.predicates import (
...
from v2.orchestrator.memory.writer import process_candidate, process_candidates
from v2.orchestrator.pipeline.parser import parse_text

REPO_ROOT = Path(__file__).resolve().parents[2]
EXTRACTION_CORPUS = REPO_ROOT / "tests" / "fixtures" / "v2_memory_extraction_corpus.json"
...
def store(tmp_path: Path) -> V2MemoryStore:
    s = V2MemoryStore(tmp_path / "v2_memory_test.sqlite")
    yield s
    s.close()
...
def test_m1_immediate_durable_writes_first_observation(store: V2MemoryStore) -> None:
...
def test_m1_call_me_jesse_writes_immediate_durable(store: V2MemoryStore) -> None:
    parsed = parse_text("call me Jesse")
    decision = process_candidate(
        {
            "subject": "self",
...
def test_m1_provisional_handle_creates_unnamed_person(store: V2MemoryStore) -> None:
...
def test_m1_provisional_merge_promotes_to_named(store: V2MemoryStore) -> None:
...
def test_m1_single_value_supersession(store: V2MemoryStore) -> None:
...
def test_m1_cross_user_isolation(store: V2MemoryStore) -> None:
...
def test_m1_pipeline_memory_write_runs_when_enabled(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "v2_pipeline_test.sqlite")
    try:
        result = run_pipeline(
...
def test_m1_pipeline_president_bug_does_not_write(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "v2_president_test.sqlite")
    try:
        result = run_pipeline(
