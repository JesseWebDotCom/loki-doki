...
class TestMigrationScript:
    """The v1 → v2 config migration script."""

    def test_script_exists(self):
        path = (
            Path(__file__).resolve().parents[2]
            / "scripts"
            / "migrate_skill_config_v1_to_v2.py"
        )
        assert path.exists()
    ...
    def test_script_importable(self):
        ...
        assert hasattr(mod, "V1_TO_V2")
        assert hasattr(mod, "migrate")

    def test_v1_mappings_cover_known_skills(self):
        import importlib.util
        path = (
            Path(__file__).resolve().parents[2]
            / "scripts"
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert "weather_openmeteo" in mod.V1_TO_V2
        assert "movies_tmdb" in mod.V1_TO_V2
        assert "movies_fandango" in mod.V1_TO_V2
