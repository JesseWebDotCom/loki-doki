...
§8 M3:
    ...
    1. v2 social reader (people + relationships from V2MemoryStore)
...
PEOPLE_CORPUS = REPO_ROOT / "tests" / "fixtures" / "v2_people_resolution_corpus.json"


@pytest.fixture()
def store(tmp_path: Path) -> V2MemoryStore:
    s = V2MemoryStore(tmp_path / "v2_memory_m3.sqlite")
    yield s
    s.close()
...
def _seed(store: V2MemoryStore, owner_user_id: int, seeds: list[dict]) -> None:
    for s in seeds:
...
def test_m3_resolver_exact_name(store: V2MemoryStore) -> None:
...
def test_m3_resolver_case_insensitive(store: V2MemoryStore) -> None:
...
def test_m3_resolver_handle_match(store: V2MemoryStore) -> None:
...
def test_m3_resolver_substring_three_chars(store: V2MemoryStore) -> None:
    _seed(store, 1, [{"subject": "person:Luke", "predicate": "is_relation", "value": "brother"}])
...
def test_m3_resolver_too_short_substring_no_match(store: V2MemoryStore) -> None:
...
def test_m3_resolver_fuzzy_typo_when_rapidfuzz_available(store: V2MemoryStore) -> None:
...
def test_m3_resolver_empty_mention_returns_no_match(store: V2MemoryStore) -> None:
    _seed(store, 1, [{"subject": "person:Luke", "predicate": "is_relation", "value": "brother"}])
...
def test_m3_resolver_unknown_person_no_match(store: V2MemoryStore) -> None:
    _seed(store, 1, [{"subject": "person:Luke", "predicate": "is_relation", "value": "brother"}])
    result = resolve_person(store, 1, "Yoda")
    assert result.matched is None


def test_m3_resolver_ambiguous_substring(store: V2MemoryStore) -> None:
...
def test_m3_relations_attached_to_hit(store: V2MemoryStore) -> None:
    _seed(
        store,
        1,
        [
...
def test_m3_combine_prompt_includes_social_context_slot() -> None:
    from v2.orchestrator.core.types import RequestSpec
...
def test_m3_pipeline_does_not_read_when_need_social_false(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m3_no_read.sqlite")
...
def test_m3_pipeline_reads_when_need_social_true(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m3_yes_read.sqlite")
    test_store.write_social_fact(
        MemoryCandidate(
            subject="person:Luke",
            predicate="is_relation",
...
def test_m3_pipeline_can_combine_need_preference_and_need_social(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m3_both.sqlite")
...
def test_m3_provisional_handle_merge_promotes_to_named(store: V2MemoryStore) -> None:
    _seed(store, 1, [{"subject": "handle:my boss", "predicate": "is_relation", "value": "boss"}])
...
def test_m3_provisional_merge_resolves_post_merge(store: V2MemoryStore) -> None:
...
def test_m3_provisional_merge_no_provisional_returns_none(store: V2MemoryStore) -> None:
...
def test_m3_idx_people_owner_handle_exists(store: V2MemoryStore) -> None:
...
def test_m3_people_resolution_corpus_top1_accuracy(people_corpus, tmp_path: Path) -> None:
    ...
    """
    test_store = V2MemoryStore(tmp_path / "m3_corpus.sqlite")
    correct = 0
    total = 0
    failures: list[str] = []
    try:
...
def test_m3_people_resolution_corpus_merge_cases(people_corpus, tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m3_merge.sqlite")
    failures: list[str] = []
    try:
        for case in people_corpus:
...
def test_m3_cross_user_isolation_in_social_read(store: V2MemoryStore) -> None:
...
def test_m3_render_social_context_clips_to_budget(store: V2MemoryStore) -> None:
...
def test_m3_pipeline_end_to_end_social_recall(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m3_e2e.sqlite")
    try:
