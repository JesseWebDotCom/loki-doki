...
from v2.orchestrator.memory.store import (
    V2MemoryStore,
...
def store(tmp_path: Path) -> V2MemoryStore:
    s = V2MemoryStore(tmp_path / "v2_memory_m25.sqlite")
    yield s
    s.close()
...
def test_m25_embedding_column_exists(store: V2MemoryStore) -> None:
...
def test_m25_embedding_populated_on_insert(store: V2MemoryStore) -> None:
...
def test_m25_vector_search_returns_relevant_hits(store: V2MemoryStore) -> None:
    store.write_semantic_fact(
        MemoryCandidate(
            subject="self",
            predicate="has_dietary_restriction",
...
def test_m25_vector_search_returns_empty_for_no_data(store: V2MemoryStore) -> None:
    hits = _vector_search(store, 999, "anything", limit=10)
    assert hits == []


def test_m25_vector_search_handles_empty_query(store: V2MemoryStore) -> None:
    store.write_semantic_fact(
        MemoryCandidate(
            subject="self",
            predicate="favorite_color",
...
def test_m25_read_user_facts_uses_three_rrf_sources(store: V2MemoryStore) -> None:
...
def test_m25_vector_source_bridges_vocabulary_mismatch(store: V2MemoryStore) -> None:
...
def test_m25_recall_corpus_bridges_dietary_query(tmp_path: Path) -> None:
    ...
    query phrasing."""
    store = V2MemoryStore(tmp_path / "m25_corpus_bridge.sqlite")
    try:
        store.write_semantic_fact(
            MemoryCandidate(
                subject="self",
...
def test_m25_vector_search_cross_user_isolation(store: V2MemoryStore) -> None:
    store.write_semantic_fact(
        MemoryCandidate(
            subject="self",
...
def test_m25_vector_search_handles_corrupt_embedding(
    store: V2MemoryStore, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_m25_pipeline_end_to_end_vocab_bridge(tmp_path: Path) -> None:
    ...
    test_store = V2MemoryStore(tmp_path / "m25_e2e.sqlite")
    try:
        # Turn 1 — write via M1.
        run_pipeline(
