"""
Tests for the dev-tools memory endpoints and the V2RunRequest memory toggles.
...
POST /dev/v2/run with memory_enabled=true must:
...
GET /dev/v2/memory/dump and POST /dev/v2/memory/reset are tested for
...
def isolated_dev_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    # there so reset_v2_memory etc. use the patched path.
...
def test_v2_run_request_default_memory_off() -> None:
    from lokidoki.api.routes.dev import V2RunRequest

    req = V2RunRequest(message="hi")
...
@pytest.mark.anyio
async def test_run_v2_pipeline_with_memory_enabled_writes_to_dev_store(
    ...
    from lokidoki.api.routes.dev import V2RunRequest, run_v2_pipeline
    ...
    result = await run_v2_pipeline(req, _=None)  # type: ignore[arg-type]
...
@pytest.mark.anyio
async def test_run_v2_pipeline_with_memory_enabled_reads_back(
    isolated_dev_store: Path,
) -> None:
...
@pytest.mark.anyio
async def test_run_v2_pipeline_with_memory_enabled_writes_person(
    isolated_dev_store: Path,
) -> None:
...
@pytest.mark.anyio
async def test_run_v2_pipeline_with_memory_disabled_is_no_op(
    isolated_dev_store: Path,
) -> None:
...
@pytest.mark.anyio
async def test_dump_v2_memory_endpoint(isolated_dev_store: Path) -> None:
    from lokidoki.api.routes.dev import V2RunRequest, dump_v2_memory, run_v2_pipeline
    ...
    payload = await dump_v2_memory(_=None)  # type: ignore[arg-type]
...
@pytest.mark.anyio
async def test_reset_v2_memory_endpoint(isolated_dev_store: Path) -> None:
    from lokidoki.api.routes.dev import V2RunRequest, reset_v2_memory, run_v2_pipeline
    ...
    summary = await reset_v2_memory(_=None)  # type: ignore[arg-type]
    assert summary["facts_cleared"] >= 1
    facts = dev_memory.get_dev_store().get_active_facts(dev_memory.DEV_OWNER_USER_ID)
    assert facts == []
