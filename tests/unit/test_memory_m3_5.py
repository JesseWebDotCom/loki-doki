...
def store(tmp_path: Path) -> MemoryStore:
    s = MemoryStore(tmp_path / "v2_memory_m35.sqlite")
    yield s
    s.close()


def _seed_handle(store: MemoryStore, owner: int, handle: str, relation: str) -> None:
...
def _name_relation(store: MemoryStore, owner: int, name: str, relation: str) -> None:
    store.write_social_fact(
        MemoryCandidate(
...
def test_m35_auto_merge_promotes_provisional_in_place(store: MemoryStore) -> None:
...
def test_m35_auto_merge_preserves_relationship_edges(store: MemoryStore) -> None:
...
def test_m35_auto_merge_resolvable_by_old_handle_and_new_name(store: MemoryStore) -> None:
    _seed_handle(store, 1, "my boss", "boss")
    _name_relation(store, 1, "Steve", "boss")
...
def test_m35_no_merge_when_no_provisional_exists(store: MemoryStore) -> None:
...
def test_m35_no_merge_when_multiple_provisional_match_same_relation(store: MemoryStore) -> None:
...
def test_m35_no_merge_when_relation_does_not_match(store: MemoryStore) -> None:
    _seed_handle(store, 1, "my therapist", "therapist")
...
def test_m35_auto_merge_does_not_cross_owner_boundary(store: MemoryStore) -> None:
    _seed_handle(store, 1, "my boss", "boss")
...
def test_m35_end_to_end_provisional_then_named(tmp_path: Path) -> None:
    ...
    test_store = MemoryStore(tmp_path / "m35_e2e.sqlite")
    try:
        run_pipeline(
