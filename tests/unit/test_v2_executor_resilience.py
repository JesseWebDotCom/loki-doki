...
from v2.orchestrator.execution.errors import HandlerError, TransientHandlerError
from v2.orchestrator.execution.executor import execute_chunk_async, register_handler
...
async def test_executor_records_timeout_failure_without_raising():
    ...
    original = v2_config.CONFIG.handler_timeout_s
    object.__setattr__(v2_config.CONFIG, "handler_timeout_s", 0.05)
    try:
        result = await execute_chunk_async(chunk, route, _selection("test.slow"), resolution)
    finally:
        object.__setattr__(v2_config.CONFIG, "handler_timeout_s", original)
