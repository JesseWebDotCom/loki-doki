...
from v2.orchestrator.fallbacks.prompts import (
    COMBINE_PROMPT,
    DIRECT_CHAT_PROMPT,
    render_prompt,
)
...
class TestAffectWindow:
    ...
    def test_upsert_overwrites(self, store: V2MemoryStore):
        store.write_affect_day(
    ...
    def test_days_limit(self, store: V2MemoryStore):
        for i in range(20):
            store.write_affect_day(
    ...
    def test_null_concerns(self, store: V2MemoryStore):
        store.write_affect_day(
    ...
    def test_empty_window(self, store: V2MemoryStore):
        rows = store.get_affect_window(OWNER, character_id="loki")
        assert rows == []
...
class TestSentimentPersistence:
    def test_seven_day_persistence(self, store: V2MemoryStore):
    ...
    def test_fourteen_day_rolling_window(self, store: V2MemoryStore):
        for i in range(14):
            store.write_affect_day(
                OWNER, character_id="loki",
                day=f"2026-04-{i+1:02d}",
...
class TestCharacterIsolation:
    def test_sentiment_isolation_by_character(self, store: V2MemoryStore):
    ...
    def test_delete_only_affects_target_character(self, store: V2MemoryStore):
        store.write_affect_day(
...
class TestSentimentOptOut:
    ...
    def test_opt_out_flag(self, store: V2MemoryStore):
    ...
    def test_opt_out_prevents_mood_slot(self, store: V2MemoryStore):
        ...
        from v2.orchestrator.memory.slots import assemble_slots
        context = {
            "memory_store": store,
            "owner_user_id": OWNER,
...
class TestForgetMyMood:
    def test_wipe_all_characters(self, store: V2MemoryStore):
    ...
    def test_wipe_single_character(self, store: V2MemoryStore):
        store.write_affect_day(
            OWNER, character_id="loki", day="2026-04-01", sentiment_avg=0.5,
        )
        store.write_affect_day(
    ...
    def test_wipe_empty_is_noop(self, store: V2MemoryStore):
        deleted = store.delete_affect_window(OWNER)
        assert deleted == 0
...
class TestRecentMoodSlot:
    ...
    def test_assemble_empty_store(self, store: V2MemoryStore):
        rendered, rows = assemble_recent_mood_slot(
            store=store, owner_user_id=OWNER, character_id="loki",
        )
        assert rendered == ""
...
class TestEpisodicCompression:
    def _insert_episode(
        self, store: V2MemoryStore, title: str, summary: str,
    ...
    def test_get_stale_episodes(self, store: V2MemoryStore):
    ...
    def test_stale_excludes_recalled(self, store: V2MemoryStore):
        old_date = "2025-09-01T00:00:00Z"
        self._insert_episode(store, "Recalled", "Stuff", old_date, recall_count=2)
        stale = store.get_stale_episodes(OWNER, older_than="2026-01-01T00:00:00Z")
        assert len(stale) == 0

    def test_compress_episodes(self, store: V2MemoryStore):
    ...
    def test_stale_excludes_already_superseded(self, store: V2MemoryStore):
...
class TestPipelineIntegration:
    def test_record_sentiment_writes_affect(self, store: V2MemoryStore):
        """Sentiment recording produces affect_window rows."""
        from v2.orchestrator.core.pipeline import _record_sentiment
    ...
    def test_opt_out_prevents_sentiment_write(self, store: V2MemoryStore):
        from v2.orchestrator.core.pipeline import _record_sentiment
    ...
    def test_sentiment_blending_within_day(self, store: V2MemoryStore):
    ...
    def test_mood_in_memory_slots(self, store: V2MemoryStore):
    ...
    def test_no_mood_without_data(self, store: V2MemoryStore):
        from v2.orchestrator.memory.slots import assemble_slots
        context = {
            "memory_store": store,
            "owner_user_id": OWNER,
...
class TestToneMapping:
    def test_all_known_tones_have_values(self):
        from v2.orchestrator.core.pipeline import _TONE_TO_SENTIMENT
