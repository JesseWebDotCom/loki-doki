...
import { ClockIcon, FolderPlusIcon, Loader2Icon, PlusIcon, Trash2Icon } from "lucide-react";
...
export function PinsPanel({
  ...
  return (
    ...
    >
      ...
      <form onSubmit={(e) => void onCreate(e)} className="grid gap-2 rounded-2xl border border-border/60 bg-card/60 p-3">
        ...
        <label className="grid gap-1 text-xs text-muted-foreground">
          ...
          <textarea
            ...
            rows={2}
            className="w-full resize-none rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-xs placeholder:text-muted-foreground/60 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          />
        </label>
      ...
      <div className="flex items-center gap-2">
        ...
        <button type="button" aria-label="Create collection" title="Create collection"
      ...
      <ul className="grid gap-2" aria-label="Saved pins list">
        ...
        {pins.map((pin) => (
          <li key={pin.pin_id} className="grid gap-1 rounded-2xl border border-border/60 bg-card/70 px-3 py-2">
            <div className="grid grid-cols-[auto_1fr_auto_auto] items-center gap-2">
              ...
              <button type="button" aria-label={`ETA to ${pin.label}`} title="ETA from here"
                className="rounded-full p-1 text-muted-foreground hover:bg-muted hover:text-foreground"
                onClick={() => void onEta(pin)}>
                <ClockIcon className="size-4" />
              </button>
              <button type="button" aria-label={`Delete pin ${pin.label}`}
                className="rounded-full p-1 text-muted-foreground hover:bg-muted hover:text-foreground"
            ...
            <textarea
              ...
              className="ml-5 w-[calc(100%-1.25rem)] resize-none rounded-md border-0 bg-transparent px-2 py-1 text-xs text-muted-foreground placeholder:text-muted-foreground/40 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring/50"
            />
          </li>
        ))}
      </ul>
