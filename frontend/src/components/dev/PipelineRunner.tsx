...
import type { PipelineRunResponse } from '../../lib/api-types';
...
/**
 * Pull a "Produced by" badge line out of a pipeline run so the reader can see
...
/**
 * Build a markdown blob containing the user prompt, the trace, and the
 ...
 */
const buildCopyBlob = (prompt: string, result: PipelineRunResponse): string => {
  const lines: string[] = [];
...
const PipelineRunner: React.FC = () => {
  ...
  // Bumped after every successful run so MemoryPanel re-fetches the
  ...
  const run = async () => {
    ...
    setError(null);
    try {
      const next = await runPipeline(message, {
        ...
        <>
          <div className="rounded-xl border border-border/30 bg-card/50 p-5 shadow-m1">
            <div className="flex items-center gap-2 border-b border-border/10 pb-4">
              ...
              <h3 className="text-lg font-bold tracking-tight">Pipeline Runner</h3>
            </div>
            <p className="mt-4 text-sm text-muted-foreground">
              Runs the deterministic request pipeline in dev-tools isolation.
            </p>
            <textarea
              value={message}
