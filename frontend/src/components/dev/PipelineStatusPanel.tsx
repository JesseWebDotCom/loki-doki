...
const PipelineStatusPanel: React.FC<Props> = ({ loading, error, status }) => {
  if (loading) {
    return (
      <div className="rounded-xl border border-border/30 bg-card/50 p-5 shadow-m1">
        <div className="text-xs italic text-muted-foreground">Loading pipeline status…</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {status.memory && (
        <div className="rounded-xl border border-border/30 bg-card/50 p-5 shadow-m1">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Memory Subsystem</div>
