...
import { getDevSkills, runDevSkill } from '../../lib/api';
import type { SkillEntry, DevSkillRunResponse, DevSkillsResponse } from '../../lib/api-types';

const DEFAULT_MESSAGE = 'hello';

const DevSkillsExplorer: React.FC = () => {
  ...
  const [runResult, setRunResult] = useState<DevSkillRunResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const response: DevSkillsResponse = await getDevSkills();
        if (cancelled) return;
  ...
  const runSkill = async () => {
    ...
    try {
      ...
      const response = await runDevSkill(selectedSkill.capability, message, parsed, resolvedTarget || undefined);
  ...
  return (
    <div className="rounded-xl border border-border/30 bg-card/50 p-5 shadow-m1">
      <div className="flex items-center gap-2 border-b border-border/10 pb-4">
        ...
        <h3 className="text-lg font-bold tracking-tight">Skills Explorer</h3>
...
export default DevSkillsExplorer;
