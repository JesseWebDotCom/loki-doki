...
const KEYFRAMES = `
@keyframes maipaitv-kb-a { from { transform: scale(1.06) translate(1.5%, 1%) } to { transform: scale(1.18) translate(-1.5%, -1.5%) } }
@keyframes maipaitv-kb-b { from { transform: scale(1.06) translate(-1.5%, -1%) } to { transform: scale(1.18) translate(1.5%, 1.5%) } }
@keyframes maipaitv-slide-fade { from { opacity: 0 } to { opacity: 1 } }
`
      ...
      {layers.map((p, li) => {
        ...
        return (
          <div
            ...
            style={li === layers.length - 1 ? { animation: 'maipaitv-slide-fade 1.6s ease-in-out both' } : undefined}
          >
            <img
              src={p.url}
              ...
              style={{ animation: `${photoIdx % 2 === 0 ? 'maipaitv-kb-a' : 'maipaitv-kb-b'} 24s ease-in-out both` }}
            />
          </div>
        )
      })}
