 ...
 *  swaps to a live audio-reactive visualizer, prominent Up Next, and lean controls. Enters
 *  the browser Fullscreen API on open so it fills the TV / display. */
export function ImmersivePlayer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [variant, setVariant] = useState<VisualizerVariant>(() =>
    (VISUALIZERS.find(v => v.id === localStorage.getItem(VIS_KEY))?.id) ?? 'aurora')
  const [idle, setIdle] = useState(false)

  const track = radio.currentTrack
  const artUrl = track?.thumbnail ? proxyImgAuto(track.thumbnail) : ''
  ...
  return createPortal(
    <div ref={rootRef}
      ...
      style={{ background: `radial-gradient(120% 120% at 50% 0%, ${palette.dark} 0%, #000 75%)` }}>

      {/* UltraBlur backdrop: the cover, massively blurred + scaled, under a dark scrim. */}
      {artUrl && (
        <div className="absolute inset-0" aria-hidden>
          {/* design-ok(hex-in-tsx): backdrop scrim alpha, not a themable colour */}
          <img src={artUrl} alt="" className="size-full scale-125 object-cover opacity-60"
            style={{ filter: 'blur(64px) saturate(1.4)' }} />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.35), rgba(0,0,0,0.75))' }} />
        </div>
      )}

      {/* Center stage: album art OR the visualizer. Tap to toggle between them. */}
      <button type="button" onClick={() => setShowVis(v => !v)}
        ...
        {showVis ? (
          <MusicVisualizer variant={variant} getAnalyser={radio.getAnalyser} palette={palette}
            active={!radio.paused} className="absolute inset-0" />
        ) : (
          artUrl
      ...
      {/* Bottom: title + progress + controls + Up Next */}
      <div className={cn('absolute inset-x-0 bottom-0 flex flex-col gap-4 p-6 pb-8 transition-opacity duration-500',
        ...
        <div className="flex items-center gap-3 text-xs tabular-nums text-white/60">
          ...
          <div className="relative h-1 flex-1 overflow-hidden rounded-full bg-white/15">
            <div className="absolute inset-y-0 left-0 rounded-full" style={{ width: `${progress * 100}%`, background: palette.light }} />
          </div>
          <span>{radio.durationSec ? fmtClock(radio.durationSec) : '--:--'}</span>
        </div>

        {/* Controls */}
        <div className="flex items-center justify-center gap-6">
          <button type="button" onClick={radio.skip} className="text-white/60 transition hover:text-white" title="Previous"><SkipBack className="size-6" /></button>
          <button type="button" onClick={radio.togglePause}
            ...
            {radio.paused ? <Play className="size-7 translate-x-0.5" /> : <Pause className="size-7" />}
          </button>
