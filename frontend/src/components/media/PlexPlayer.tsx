  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-black/95 backdrop-blur-sm">
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <button onClick={onClose} className="rounded-full p-1.5 text-white/70 transition-colors hover:bg-white/10 hover:text-white" aria-label="Close player">
          <X className="size-5" />
        </button>
      </div>
      <div className="flex min-h-0 flex-1 items-center justify-center px-4 pb-4">
        {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
        <video src={plexStreamUrl(ratingKey)} controls autoPlay className="max-h-full max-w-full rounded-lg" />
      </div>
    </div>
  )
}
