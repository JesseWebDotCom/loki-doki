...
export function SettingsAboutTab() {
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h2 className="text-base font-semibold">About loki-doki</h2>
      ...
      <div>
        <h3 className="mb-3 text-sm font-medium">Open-source & content licenses</h3>
        ...
        <div className="divide-y divide-border rounded-lg border">
          {LICENSES.map((item) => (
            <div key={item.name} className="flex flex-col gap-0.5 px-3 py-2.5 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span className="shrink-0 text-right text-xs text-muted-foreground/70 sm:ml-4">
                {item.license}
              </span>
            </div>
          ))}
