...
import { Captions, Ear, Square, Volume2 } from 'lucide-react'
...
import { type CaptionStyle } from '@/lib/companionState'
...
export function Indicator({ icon: Icon, label, state, onClick }: { icon: typeof Ear; label: string; state: IndicatorState; onClick?: () => void }) {
...
export function CompanionDock({ collapsed }: { collapsed?: boolean }) {
  ...
  // A pending confirmation keeps it open past the linger: the offer's buttons
  // must not vanish while the user is deciding.
  const bubbleActive = thinking || !!captionText || !!engine.pendingAction
    ...
    <>
      ...
      {bubbleShown && anchor && createPortal(
        <div
          ...
          className={cn(
            'max-w-sm rounded-sheet border border-border bg-popover px-4 py-3 shadow-lg transition-opacity duration-200',
            // The bubble is decorative (pointer-events-none) EXCEPT while a
            // confirmation offer's buttons are live.
            engine.pendingAction ? 'pointer-events-auto' : 'pointer-events-none',
