    ...
    <>
      {bannerUrl && bannerOk && (
        <div className="mb-5 overflow-hidden rounded-card ring-1 ring-border/40">
          <img src={proxyImage(bannerUrl)} alt="" referrerPolicy="no-referrer" className="aspect-[6/1] w-full object-cover" onError={() => setBannerOk(false)} />
        </div>
      )}
      <div className="mb-6 flex items-start gap-4">
        <CreatorAvatar title={title} src={avatarUrl} proxy={proxyImage} className="size-20 shrink-0 text-3xl ring-1 ring-border/40 sm:size-24" />
