...
import { Bookmark, CheckCheck, ThumbsUp, ThumbsDown, Sparkles } from 'lucide-react'
...
function FeedThumb({ item }: { item: FeedItem }) {
  if (item.imageUrl) return <img src={proxyImg(item.imageUrl)} alt="" className="size-20 shrink-0 rounded-control object-cover" loading="lazy" onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
  return null
}
...
export function FeedListView({ scope }: { scope: FeedScope }) {
  ...
  return (
    <div>
      ...
      <div className="divide-y divide-border/40 rounded-card border border-border/40">
        {items.map(item => (
          <button key={item.id} onClick={() => open(item)}
            ...
            <div className="flex shrink-0 flex-col items-center gap-1">
              <button onClick={(e) => toggleSave(e, item)} title={item.saved ? 'Saved' : 'Save to Bookmarks'}
                className={cn('rounded-control p-1.5 transition-colors', item.saved ? 'text-primary' : 'text-muted-foreground hover:text-foreground')}>
                <Bookmark className={cn('size-4', item.saved && 'fill-current')} />
              </button>
              <div className="flex gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
                <button onClick={(e) => feedback(e, item, 'up')} title="More like this" className="rounded p-1 text-muted-foreground hover:text-success"><ThumbsUp className="size-3.5" /></button>
                <button onClick={(e) => feedback(e, item, 'down')} title="Less like this" className="rounded p-1 text-muted-foreground hover:text-destructive"><ThumbsDown className="size-3.5" /></button>
              </div>
            </div>
          </button>
        ))}
