...
@keyframes dokitv-hearth-glow {
  0% { opacity: 0.55; }
  50% { opacity: 0.85; }
  100% { opacity: 0.6; }
}
@keyframes dokitv-ember {
...
function flameStyle(channel: TvChannel, spec: FlameSpec): CSSProperties {
  return {
    ...
    animation: `dokitv-flame ${spec.duration} ease-in-out infinite alternate`,
    ...
    <div data-theme="dark" className="relative h-full w-full overflow-hidden bg-black text-foreground">
      ...
      <div
        ...
        style={{
          ...
          animation: 'dokitv-hearth-glow 5s ease-in-out infinite',
        }}
      />
      ...
      {EMBERS.map((spec, i) => (
        <span
          ...
          style={{
            ...
            animation: `dokitv-ember ${spec.duration} linear infinite`,
            animationDelay: spec.delay,
          }}
        />
      ))}
