...
const PROSE = cn(
  ...
  '[&_img]:my-5 [&_img]:rounded-xl [&_img]:max-w-full [&_img]:h-auto [&_img]:mx-auto',
  '[&_figure]:my-5 [&_figcaption]:mt-2 [&_figcaption]:text-center [&_figcaption]:text-xs [&_figcaption]:text-muted-foreground',
  '[&_pre]:my-4 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-[#1e1e1e] [&_pre]:p-4 [&_pre]:text-[13px]',
    ...
    <article className={cn('px-5 py-8', className)}>
      ...
      {contentHtml ? (
        ...
        <div className="max-w-[44rem] mx-auto text-muted-foreground">
          ...
          {url && (
            <a href={url} target="_blank" rel="noopener noreferrer" className="text-violet-400 underline">
              Open the original
            </a>
          )}
          .
