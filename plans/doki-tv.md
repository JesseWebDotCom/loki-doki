# Doki TV: linear family channels plan

A full linear-TV experience inside Loki Doki: numbered channels, a guide, channel surfing,
...
Plex. We own our client. So Doki TV splits into:
...
| # | Channel | What it shows | Infra |
|---|---|---|---|
| 11 | Doki FM | Music stations with `AudioVisualizer` scenes, AI DJ talkover interstitials | music stack, `musicRadio.ts` |
...
- **Nightly sign-off**: anthem, test pattern, "Doki TV has concluded its broadcast day"
...
- Admin: Doki TV settings page (`AppSettingsShell`): channel manager (create/reorder/
...
prove each renderer family: Marathon (media), Doki FM (audio), Weather (page). Filler =
static bumper cards.
...
- Frontend `/channels` app (label "Doki TV", registry id `dokitv`): dark-shell layout,
