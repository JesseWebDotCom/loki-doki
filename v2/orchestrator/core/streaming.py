"""SSE streaming wrapper for the v2 pipeline.
...
from v2.orchestrator.core.types import TraceStep
...
# Which v1 phase each v2 trace step contributes timing to.
...
async def stream_pipeline_sse(
    ...
    """Run the v2 pipeline and yield v1-compatible SSE events.
    ...
    # Cache each v2 step so "done" builders can read sibling details.
    ...
    def _on_step(step: TraceStep) -> None:
        ...
        # ---- phase transitions at specific v2 step boundaries ----
    ...
    async def _run() -> None:
        try:
            ...
            logger.exception("v2 pipeline crashed during SSE stream")
...
def _build_synthesis_done(result: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "model": getattr(result.request_spec, "llm_model", None) or "v2-pipeline",
