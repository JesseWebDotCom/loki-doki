"""
v2 memory storage layer — clean cutover from v1.

The v2 memory store opens its **own** SQLite file at
``data/v2_memory.sqlite`` (configurable per-instance) and writes to its
own tables. It imports zero v1 modules. v1's memory at
``lokidoki/core/memory_*`` and its SQLite file are untouched. A clean
cutover from v1 to v2 is "delete v1 modules, point the app at this store"
...
remaining tables via :func:`apply_v2_memory_schema` against this same
file.
"""
...
DEFAULT_DB_PATH = Path("data/v2_memory.sqlite")


# ---------------------------------------------------------------------------
# Schema (v2-owned, separate from v1)
# ---------------------------------------------------------------------------

V2_MEMORY_CORE_SCHEMA: str = """
...
class WriteOutcome:
    """Result of a single write through the v2 store."""
...
class V2MemoryStore:
    """Thread-safe SQLite store for the v2 memory subsystem.
    ...
    def _bootstrap(self) -> None:
        with self._lock:
            self._conn.executescript(V2_MEMORY_CORE_SCHEMA)
...
_GLOBAL_STORE: V2MemoryStore | None = None
_GLOBAL_LOCK = threading.Lock()


def get_default_store() -> V2MemoryStore:
    """Return the process-wide default store, creating it on first call.

    Tests typically construct their own ``V2MemoryStore(":memory:")`` so
    they don't share state with the runtime singleton.
    """
    global _GLOBAL_STORE
    with _GLOBAL_LOCK:
        if _GLOBAL_STORE is None:
            _GLOBAL_STORE = V2MemoryStore()
        return _GLOBAL_STORE


def reset_default_store(new_store: V2MemoryStore | None = None) -> None:
