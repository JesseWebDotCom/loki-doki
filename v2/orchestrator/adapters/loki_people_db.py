"""SQLite-backed PeopleDB adapter for the v2 orchestrator.

Reads the same ``people`` / ``relationships`` tables that the legacy
LokiDoki memory subsystem writes to, but exposes the v2 prototype's
...
— v2 must never mutate the legacy database. Per CLAUDE.md, the v2
...
from v2.orchestrator.adapters.people_db import PersonMatch, PersonRecord
