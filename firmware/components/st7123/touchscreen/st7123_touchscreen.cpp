...
// Registers (16-bit addressed). Matches M5GFX Touch_ST7123 + espp st7123touch.
...
void ST7123Touchscreen::setup() {
  if (this->reset_pin_ != nullptr) {
    // reset_pin_ is the ST7123 touch ENABLE line (BSP_TOUCH_EN, expander 0x43 P5).
    // It is an ENABLE/strobe, not a reset: drive a clean disable→enable (low→high)
    // edge to boot the touch firmware so it actually scans and reports coordinates.
    // Timing matches espp/esp-bsp (20 ms low, 50 ms after enable) — the shorter pulse
    // the upstream component shipped was not always enough to bring the engine up.
    this->reset_pin_->setup();
    this->reset_pin_->digital_write(false);  // disable
    delay(20);
    this->reset_pin_->digital_write(true);   // enable
...
void ST7123Touchscreen::dump_config() {
  ...
  LOG_PIN("  Enable Pin: ", this->reset_pin_);
}
